...
- **Unity 控制台日志过滤**（`SuppressUnityConsoleWarnings`，默认 `true`）：启用后过滤所有无 `[yyyy-MM-dd HH:mm:ss]` 前缀的 stdout/Unity 输出；仅保留 `CommandWindow` 与 `Rocket.log`。旧键 `SuppressHeadlessGraphicsLogs` 仍兼容。
