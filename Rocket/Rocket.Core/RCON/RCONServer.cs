...

{
...

    {
        ...
        private sealed class PendingRconCommand

        {

            public RCONConnection Client = null!;
        ...
        private static Queue<PendingRconCommand> commands = new Queue<PendingRconCommand>();



        // Optional preprocessor (e.g. command alias expansion) applied before Execute.
        public static Func<string, string>? PreprocessCommand;
...

            {
...

                {
...

                    {
                        ...
                        newclient.Send(newclient.Interactive

                            ? "Notice: Interactive mode enabled. Command output will be streamed.\r\n"

                            : "Notice: Interactive mode disabled.\r\n");
...

                    {
                        ...
                        commands.Enqueue(new PendingRconCommand { Client = newclient, Command = command });
...

        {
...

            {
...

                {
                    ...
                    PendingRconCommand pending = commands.Dequeue();

                    if (pending.Client == null || !pending.Client.Client.Client.Connected || !pending.Client.Authenticated)
                    ...
                    string raw = pending.Command.Trim();
                    ...
                    string expanded = PreprocessCommand != null ? PreprocessCommand(raw) : raw;
...

                    {
                        ...
                        pending.Client.Send(R.Translate("command_executed"));
...

                    {
...

                        {
                            ...
                            pending.Client.Send(R.Translate("command_not_found"));
...

                        {
                            ...
                            pending.Client.Send(R.Translate("command_failed"));
